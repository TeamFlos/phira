...
/// Requests the highest available display refresh rate on Android.
///
/// On Android, the system's power-saving strategy may cap the display refresh
/// rate at 60 Hz for apps that don't explicitly request a higher mode. This
/// function queries the display's supported modes and sets
/// `preferredDisplayModeId` on the Activity's Window to the highest available
/// refresh rate, making the game run at the panel's native refresh rate
/// (e.g. 120 Hz) without requiring a global system override.
#[cfg(target_os = "android")]
