...
impl EnergyDiff {
    pub fn new(pcm: &[f32], sample_rate: u32, frame_ms: f64, hop_ms: f64) -> Self {
        ...
        debug_assert!(frame_ms.is_finite() && frame_ms > 0.0, "frame_ms must be finite and positive");
        debug_assert!(hop_ms.is_finite() && hop_ms > 0.0, "hop_ms must be finite and positive");
