...
impl AVFormatContext {
    pub fn new() -> Result<Self> {
        unsafe {
            Ok(Self(
                OwnedPtr::new(ffi::avformat_alloc_context())
                    .context("failed to allocate format context")?,
            ))
        }
    }
    ...
    pub fn open_input(&mut self, url: &str) -> AVResult<()> {
        unsafe {
            ...
            handle(ffi::avformat_open_input(
                self.0.as_self_mut(),
                url.as_ptr(),
                null_mut(),
                null_mut(),
    ...
    pub fn streams(&self) -> Vec<AVStreamRef> {
        unsafe {
            ...
            std::slice::from_raw_parts(this.streams as *const AVStreamRef, this.nb_streams as _)
                .to_vec()
        }
    }
