...
item-offline-sub = 모든 온라인 기능을 비활성화합니다.
item-server-status = 서버 상태
item-server-status-sub = 웹페이지로 이동하여 서버 상태를 확인하세요.
check-status = 확인
item-mp = 멀티플레이어
item-mp-sub = 멀티플레이 기능을 활성화합니다.
item-mp-addr = 멀티플레이어 서버
item-mp-addr-sub = 사용자 지정 멀티플레이 서버에 연결합니다.
...
item-clear-cache-btn = 삭제
item-cache-cleared = 캐시 삭제됨
item-insecure = 보안 모드 끄기
item-insecure-sub = 구형 장치에서 온라인 기능을 사용할 수 있도록 합니다.
item-enable-anys = Anys 활성화
item-enable-anys-sub = 연결 품질을 향상시키기 위해 Anys 게이트웨이를 사용합니다
item-anys-gateway = Anys 게이트웨이
item-anys-gateway-sub = 사용자 지정 Anys 게이트웨이 주소를 사용합니다.
...
item-dhint = 동시 입력 힌트
item-dhint-sub = 동시에 눌러야 하는 노트를 강조 표시합니다.
item-opt = 차트 최적화
item-opt-sub = 플레이 중 성능을 크게 향상시킵니다. (예기치 않은 문제가 발생하면 이 옵션을 비활성화하세요.)
item-speed = 속도
item-note-size = 노트 크기

item-chart-debug = 라인 ID 표시
item-chart-debug-sub = 라인의 ID와 방향을 표시합니다.
item-touch-debug = 터치 포인트 표시
item-touch-debug-sub = 사용자의 터치 지점을 표시합니다.
...
about-content =
  ...
  Phira는 Phigros에서 영감을 받은 비영리 커뮤니티 주도 리듬 게임입니다.

  BiliBili Account: @Phira官方
  QQ Guild: r48eajexth
  Discord Server: discord.gg/gqpR3bTSsP

  실시간 업데이트를 받고 도움을 받으려면 QQ 길드 또는 Discord 서버에 참여하는 것을 권장합니다.
