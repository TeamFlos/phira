
load-preview-failed = Échec du chargement de l'aperçu du morceau
load-charts-failed = Impossible de charger les partitions
no-chart-for-download = Cette partition n'est pas téléchargeable

load-chart-failed = Impossible de charger la partition

dl-cancel = Annuler
dl-status-fetch = Récupération d'informations
dl-status-song = Téléchargement de chanson
dl-status-chart = Téléchargement des partitions
dl-status-extract = Extraction
dl-status-illustration = Téléchargement de l'illustration
dl-status-assets = Téléchargement de ressources
dl-status-saving = Enregistrement
dl-failed = Échec du téléchargement
dl-success = Téléchargé

guest = Touriste

warn-unrated = Ce jeu n'est pas noté
failed-to-play = Impossible de démarrer en mode en ligne
play-cancel = Annuler
play-switch-to-offline = Passer en mode hors-ligne
switched-to-offline = Passé en mode hors-ligne

delete = Supprimer
rate = Noter
exercise = Exercer
offset = Ajuster le décalage

edit-cancel = Annuler
edit-save = Entrgistrer
edit-saving = Enregistrement
edit-load-file-failed = Impossible de charger le fichier externe
edit-save-failed = Échec de l'enregistrement
edit-saved = Enregistré
edit-preview-invalid = Temps d'aperçu hors limites
edit-tags = Modifier les étiquettes
edit-downloaded = Vous ne pouvez pas modifier la partition téléchargée

edit-upload = Télécharger
upload-login-first = Connectez-vous d'abord, s'il vous plaît
upload-builtin = Les partitions intégrées ne peuvent pas être téléchargées
upload-downloaded = Les partitions téléchargées ne peuvent pas être téléchargées
upload-rules = À lire avant de télécharger votre contenu
upload-rules-content =
  Avant de télécharger, vous devez confirmer que :
  1. La partition a été créée par moi, ou l'acte de la télécharger a été approuvé par l'auteur. Si c'est le deuxième cas, il faut indiquer dans l'introduction de la partition qu'elle est téléchargée au nom d'une personne ;
  2. Si la partition est créée par vous-même, il est recommandé d'utiliser un avatar et un identifiant hautement reconnaissables ; Si l'avatar et l'identifiant que vous utilisez sur prpr sont assez différents de la plate-forme de publication de vidéos de partition (en particulier, Bilibili), des annotations supplémentaires sont nécessaires ;
  3. ! Il est interdit de télécharger la partition sans le consentement de l'auteur, sinon vous risquez la peine d'une interdiction permanente de téléchargement ;
  4. Le contenu de la partition (y compris la musique, les illustrations, le texte, etc.) doit être conforme aux autres lois et réglementations de la République populaire de Chine, et ne doit pas violer la loi ou contenir de mauvaises informations.
upload-cancel = Annuler
upload-confirm = Confirmer
uploading = Téléchargement
upload-chart-failed = Échec du téléchargement de la partition
upload-success = Téléchargé avec succès, veuillez attendre l'examen !
upload-failed = Échec du Téléchargement

ldb = Classement
ldb-load-failed = Échec du chargement du classement
ldb-no-rank = Aucun
ldb-score = Score
ldb-std = Pureté

info-name = Nom
info-composer = Compositeur
info-charter = Auteur
info-difficulty = Difficulté
info-desc = Description

review-approve = Approuver
review-deny = Refuser
review-del = Supprimer en ligne
review-approved = Approuvé
review-passed = La partition téléchargée est acceptée
review-denied = Refusé
review-deleted = Supprimé
review-action-failed = Échec de l'exécution de l'action d'examen
review-doing = Exécution de l'action
review-not-loaded = Les informations ne sont pas chargées, veuillez patienter
review-edit-tags = Modifier les étiquettes
review-edit-tags-failed = Échec de la modification des étiquettes
review-edit-tags-done = Étiquettes mises à jour

mods = Mods
mods-autoplay = Autoplay
mods-autoplay-sub = Après l'activation, il ne sera pas possible de télécharger les notes
mods-flip-x = Flip X
mods-flip-x-sub = Inversion de la partition sur l'axe des abscisses
mods-fade-out = Fade out
mods-fade-out-sub = Les notes disparaissent à l'approche de la ligne de jugement

rate-failed = Échec de la notation
rate-done = Noté avec succès
